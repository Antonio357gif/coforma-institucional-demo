...
const estadoOperativoOptions: SelectOption[] = [
  { value: "todos", label: "Todos" },
  { value: "pendiente_ejecutar", label: "Pendiente de ejecutar" },
...
function pagoLabel(value: string | null | undefined) {
  return labelFromOptions(pagoOptions, value ?? "");
}
...
function defaultTecnicoForAccion(accion: AccionResumenRow | null) {
...
function accionSemantica(accion: AccionResumenRow | null) {
  if (!accion) return "Seleccione una acción para ver su regla operativa.";

  if (accion.estado_operativo_administrativo === "finalizada") {
    return "Finalizada: documentación completa validada y trazada. Pago administrativo pagado.";
  }
  ...
  if (accion.estado_operativo_administrativo === "pendiente_ejecutar") {
    return "Pendiente de ejecutar: sin ejecución efectiva. Documentación no aplica y pago no devengado.";
  }

  return "Revise la situación operativa antes de actuar.";
}
...
export default function RecepcionDocumentacionPage() {
  ...
  const filtrosActivos = useMemo(() => {
    ...
    if (busquedaAplicada.trim() !== "") activos.push(`Búsqueda: ${busquedaAplicada.trim()}`);
    if (tipoFiltro !== "todos") activos.push(`Tipo: ${tipoFiltro}`);
    if (estadoOperativoFiltro !== "todos") {
      activos.push(`Estado: ${labelFromOptions(estadoOperativoOptions, estadoOperativoFiltro)}`);
    }
  ...
  async function loadResumen(intentos = 0) {
    ...
    if (error) {
      if (intentos < 1) {
        setResumenIntentos((current) => current + 1);

        window.setTimeout(() => {
          loadResumen(intentos + 1);
        }, 700);
    ...
    try {
      const { data, error } = await supabase.rpc(
        "get_recepcion_documentacion_acciones_pagina_filtrada_v2",
        {
          p_limit: pageSize + 1,
          p_offset: inicio,
          p_busqueda: busquedaAplicada.trim(),
          p_tipo_oferta: tipoFiltro,
          p_estado_operativo: estadoOperativoFiltro,
  ...
  useEffect(() => {
    ...
    setPagoDraft(selectedAccion?.estado_pago_administrativo ?? "");
    setPagoObservacion("");
    setPagoSaved(false);
    setPagoError(null);
    setFaseActiva("todos");
  }, [selectedAccion?.subexpediente_id, selectedAccion?.estado_pago_administrativo]);
  ...
  function toggleRiesgoActivo() {
    setSoloRiesgoActivo((current) => !current);
    setPagina(1);
    setSelectedAccion(null);
  }
  ...
  function abrirMesa(accion: AccionResumenRow, fase: string = "todos") {
  ...
  function prepararAccionRapida(row: RecepcionRow, estado: string) {
    const tecnico =
      estado === "no_aplica"
        ...
        : drafts[row.recepcion_id]?.tecnico_revisor ||
          row.tecnico_revisor ||
          defaultTecnicoForAccion(selectedAccion);

    const observacionBase = drafts[row.recepcion_id]?.observaciones ?? row.observaciones ?? "";
    const textoAccion =
      estado === "recibido"
        ? "Recepción documental registrada desde mesa técnica."
        : estado === "validado"
        ? "Documento validado desde mesa técnica."
        : estado === "subsanable"
        ? "Documento marcado para subsanación desde mesa técnica."
        : estado === "no_aplica"
        ? "Documento marcado como no aplica desde mesa técnica."
        : "Cambio documental registrado desde mesa técnica.";
    ...
    updateDraft(row.recepcion_id, {
      ...
      observaciones: [observacionBase, textoAccion].filter(Boolean).join("\n"),
    });
  }
  ...
  async function guardarPago(nuevoEstado?: string, observacionForzada?: string) {
    ...
    const observacion =
      ...
      `Cambio de estado de pago desde mesa documental: ${pagoLabel(
        selectedAccion.estado_pago_administrativo
      )} → ${pagoLabel(nextPago)}.`;
    ...
    const { error } = await supabase
      ...
      .eq("id", selectedAccion.subexpediente_id);
    ...
    setAcciones((current) =>
      current.map((accion) =>
        accion.subexpediente_id === selectedAccion.subexpediente_id
          ? {
              ...accion,
              estado_pago_administrativo: nextPago,
            }
    ...
    await loadDetalle(selectedAccion.subexpediente_id);
    loadResumen(0);
    loadAcciones();
  ...
  const selectedDetalleAgrupado = useMemo(() => {
    const grupos = detallePorFase(detalleRows);
    if (faseActiva === "todos") return grupos;
    return grupos.filter((group) => group.fase === faseActiva);
  }, [detalleRows, faseActiva]);

  const resumenTotalTexto = loadingResumen
    ? "Cargando resumen..."
    : resumenDisponible
    ? `${num(resumenCalculado.subexpedientes)} subexpedientes · ${num(resumenCalculado.entidades)} entidades`
    : "Resumen pendiente de recarga";
      ...
      <section className="mx-auto max-w-7xl space-y-2 px-5 py-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="rounded-full border border-slate-200 bg-white px-2.5 py-0.5 text-[10px] font-semibold text-slate-600 shadow-sm">
              {resumenDisponible
                ? `${num(resumenCalculado.ofertas)} ofertas · ${num(
                    resumenCalculado.subexpedientes
                  )} subexpedientes · ${num(resumenCalculado.entidades)} entidades`
                : "Resumen global pendiente"}
            </span>
        ...
        {resumenError ? (
          <section className="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-[11px] text-amber-900 shadow-sm">
            El resumen global no cargó en el primer intento. No se muestran ceros falsos como dato real.
            <button
              type="button"
              onClick={() => loadResumen(0)}
        ...
        {loadingResumen && resumenIntentos > 0 ? (
          <section className="rounded-xl border border-blue-200 bg-blue-50 px-3 py-2 text-[11px] text-blue-900 shadow-sm">
            Reintentando carga del resumen global...
          </section>
        ) : null}
...
/>
          ...
          <KpiCard
            label="Riesgo activo"
            value={resumenDisponible ? num(resumenCalculado.riesgoActivoAltoCritico) : "—"}
            detail={
              resumenDisponible
                ? `${num(resumenCalculado.riesgoPrioritarioAltoCritico)} priorit. · ${num(
                    resumenCalculado.riesgoVencidoAltoCritico
                  )} vencidos`
                : "pendiente"
            }
            tone="red"
          ...
          <div className="grid gap-2 lg:grid-cols-[1.25fr_0.45fr_0.75fr_0.75fr_0.75fr_auto_auto]">
            <div>
              ...
              >
                <input
                  ...
                  placeholder="Entidad, CIF, acción, especialidad, denominación..."
                  className="h-8 w-full rounded-lg border border-slate-200 bg-slate-50 px-3 text-xs outline-none focus:border-blue-400 focus:bg-white"
                />
                <button
                  type="submit"
                  className="h-8 rounded-lg border border-blue-200 bg-blue-50 px-3 text-[11px] font-semibold text-blue-800 hover:bg-blue-100"
                >
                  Buscar
                </button>
              </form>
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  setEstadoOperativoFiltro(event.target.value);
                  setPagina(1);
                  setSelectedAccion(null);
                }}
                className="mt-1 h-8 w-full rounded-lg border border-slate-200 bg-slate-50 px-2 text-xs outline-none focus:border-blue-400 focus:bg-white"
            ...
            <div>
              ...
              <select
                value={lecturaFiltro}
                onChange={(event) => {
                  setLecturaFiltro(event.target.value);
                  setPagina(1);
                  setSelectedAccion(null);
                }}
                className="mt-1 h-8 w-full rounded-lg border border-slate-200 bg-slate-50 px-2 text-xs outline-none focus:border-blue-400 focus:bg-white"
            ...
            <div className="flex items-end">
              <button
                type="button"
                onClick={toggleRiesgoActivo}
                className={
                  soloRiesgoActivo
                    ? "h-8 rounded-lg border border-red-300 bg-red-50 px-3 text-[11px] font-semibold text-red-800"
          ...
          {filtrosActivos.length > 0 ? (
            <div className="mt-2 flex flex-wrap gap-1 text-[10px] text-slate-600">
              ...
              {filtrosActivos.map((filtro) => (
                <span
                  key={filtro}
                  className="rounded-full border border-blue-100 bg-blue-50 px-2 py-0.5 text-blue-800"
                >
                  {filtro}
                </span>
              ))}
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-3 py-1.5">
              <div>
                ...
                <p className="text-[10.5px] leading-4 text-slate-500">
                  Una línea por subexpediente. La tabla es paginada; los KPI superiores representan el resumen global.
                </p>
              </div>
              ...
              <table className="w-full table-fixed border-collapse text-left text-[11px]">
                <thead className="sticky top-0 z-10 bg-slate-50 text-[9.5px] uppercase tracking-wide text-slate-500">
                  <tr>
                    ...
                    <th className="w-[28%] px-2 py-1.5">Fases clicables</th>
                ...
                <tbody>
                  ...
                  {!loadingAcciones && acciones.length === 0 ? (
                    <tr>
                      <td colSpan={5} className="px-3 py-8 text-center text-xs text-slate-500">
                        No hay subexpedientes que coincidan con los filtros aplicados.
                      </td>
                    </tr>
                  ) : null}
                          ...
                          <td className="px-2 py-1.5 align-top">
                            <div className="flex flex-col gap-1">
                              ...
                              <SmallBadge className={pagoBadgeClass(accion.estado_pago_administrativo)}>
                                Pago: {pagoLabel(accion.estado_pago_administrativo)}
                              </SmallBadge>
                            </div>
                          </td>
            ...
            <div className="border-b border-slate-100 px-3 py-2">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-[10.5px] leading-4 text-slate-500">
                    Revisión por fases, actuación técnica y estado de pago del subexpediente seleccionado.
                  </p>
                </div>
                ...
                <div className="mt-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
                  <div className="flex flex-wrap items-start justify-between gap-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                      ...
                      <SmallBadge className={pagoBadgeClass(selectedAccion.estado_pago_administrativo)}>
                        Pago: {pagoLabel(selectedAccion.estado_pago_administrativo)}
                      </SmallBadge>
                    </div>
                  </div>
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="max-w-2xl">
                        <p className="text-[10.5px] font-semibold text-slate-800">
                          Regla operativa de esta acción
                        </p>
                        <p className="mt-0.5 text-[10px] leading-4 text-slate-500">
                          {accionSemantica(selectedAccion)}
                        </p>
                      </div>

                      <div className="grid min-w-[320px] gap-1">
                        <label className="text-[9px] font-semibold uppercase tracking-wide text-slate-500">
                        ...
                        <div className="flex gap-1">
                          <select
                            ...
                            className="h-8 flex-1 rounded-lg border border-slate-200 bg-white px-2 text-[10.5px] font-semibold outline-none focus:border-blue-400"
                          >
                          ...
                          <button
                            ...
                            disabled={
                              savingPago ||
                              !pagoDraft ||
                              pagoDraft === selectedAccion.estado_pago_administrativo
                            }
                            onClick={() => guardarPago()}
                            className={
                              pagoDraft !== selectedAccion.estado_pago_administrativo
                                ? "h-8 rounded-lg bg-[#183B63] px-3 text-[10px] font-semibold text-white hover:bg-[#102a47] disabled:opacity-60"
                                : "h-8 rounded-lg border border-slate-200 bg-slate-50 px-3 text-[10px] font-semibold text-slate-400"
                            }
                          >
                            {savingPago ? "Guardando..." : pagoSaved ? "Guardado" : "Guardar pago"}
                          </button>
                        </div>
                        ...
                        <textarea
                          ...
                          rows={2}
                          placeholder="Observación técnica del cambio de pago..."
                        ...
                        {canAutorizarPago(selectedAccion) ? (
                          <button
                            type="button"
                            disabled={savingPago}
                            onClick={() => {
                              ...
                              guardarPago(
                                "pagado",
                                "Pago autorizado desde mesa documental tras documentación completa validada y sin riesgo activo."
                              );
                            }}
                            className="rounded-lg border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[10px] font-semibold text-emerald-800 hover:bg-emerald-100 disabled:opacity-60"
                          >
                            Autorizar pago
                          </button>
                        ) : null}
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    <button
                      ...
                      className={
                        faseActiva === "todos"
                          ? "rounded-lg bg-[#183B63] px-2 py-1 text-[9.5px] font-semibold text-white"
                          : "rounded-lg border border-slate-200 bg-white px-2 py-1 text-[9.5px] font-semibold text-slate-700 hover:bg-slate-50"
                      }
                    >
                      Todas las fases
                    </button>
                    ...
                    {faseOrder.map((fase) => (
                      <button
                        ...
                        className={`rounded-lg border px-2 py-1 text-center text-[9.5px] font-semibold hover:brightness-95 ${faseResumenClass(
                          selectedAccion,
                          fase
                        )}`}
                      >
                        {labelFromOptions(faseOptions, fase)} · {faseResumenText(selectedAccion, fase)}
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="max-h-[720px] overflow-auto px-3 py-2">
              {!selectedAccion ? (
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-6 text-center text-xs text-slate-500">
                  Seleccione una acción pulsando Abrir mesa o una fase clicable para revisar su documentación.
                </div>
              ) : null}
                        ...
                        {faseRows.map((row) => {
                          ...
                          return (
                            <div
                              ...
                              className={
                                hasChanges
                                  ? "grid gap-2 bg-amber-50/50 px-3 py-2 lg:grid-cols-[1.2fr_0.74fr_1fr_auto]"
                                  : "grid gap-2 px-3 py-2 hover:bg-blue-50 lg:grid-cols-[1.2fr_0.74fr_1fr_auto]"
                              }
                            >
                              <div>
                                ...
                                <p className="text-[10px] leading-4 text-slate-500" title={clean(row.actuacion_sugerida)}>
                                  Actuación sugerida: {shortText(row.actuacion_sugerida, 92)}
                                </p>
                              </div>
                                ...
                                <div className="grid gap-1">
                                  ...
                                  <div className="grid grid-cols-2 gap-1">
                                    <button
                                      ...
                                      className="rounded-lg border border-blue-200 bg-blue-50 px-2 py-1 text-[9.5px] font-semibold text-blue-800 hover:bg-blue-100"
                                    >
                                      Registrar recepción
                                    </button>
                                    <button
                                      ...
                                      className="rounded-lg border border-emerald-200 bg-emerald-50 px-2 py-1 text-[9.5px] font-semibold text-emerald-800 hover:bg-emerald-100"
                                    >
                                      Validar
                                    </button>
                                    <button
                                      ...
                                      className="rounded-lg border border-orange-200 bg-orange-50 px-2 py-1 text-[9.5px] font-semibold text-orange-800 hover:bg-orange-100"
                                    >
                                      Subsanar
                                    </button>
                                    <button
                                      ...
                                      className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-[9.5px] font-semibold text-slate-700 hover:bg-slate-100"
                                    >
                                      No aplica
                                    </button>
                                  </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="flex flex-wrap gap-2 text-[10px]">
              ...
              <button
                type="button"
                onClick={scrollToMesa}
                className="rounded-full border border-violet-200 bg-violet-50 px-2.5 py-0.5 font-semibold text-violet-800 hover:bg-violet-100"
              >
